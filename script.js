...
function renderNutrition(recipe) {
  ...
  const proteinCalories = Number(nutrition.protein) * 4;
  const carbsCalories = Number(nutrition.carbs) * 4;
  const fatCalories = Number(nutrition.fat) * 9;
  const macroCalories = proteinCalories + carbsCalories + fatCalories;
  const totalCalories = Number(nutrition.calories) || Math.round(macroCalories);
  const proteinPercent = macroCalories ? proteinCalories / macroCalories * 100 : 0;
  const carbsEnd = macroCalories ? proteinPercent + carbsCalories / macroCalories * 100 : 0;
  const chartStyle = `conic-gradient(var(--nutrition-protein) 0% ${proteinPercent}%, var(--nutrition-carbs) ${proteinPercent}% ${carbsEnd}%, var(--nutrition-fat) ${carbsEnd}% 100%)`;
  nutritionBox.innerHTML = `<div class="nutrition-header"><div><span class="nutrition-kicker">PER SERVING</span><h3>Nutrition at a Glance</h3></div><span class="nutrition-estimate">Estimated values</span></div><div class="nutrition-visual"><div class="macro-chart" role="img" aria-label="Macronutrient chart showing the relative calories from protein, carbohydrates and fat" style="background:${chartStyle}"><span><strong>${totalCalories}</strong><small>kcal</small></span></div></div><div class="nutrition-grid"><div class="nutrition-item nutrition-protein"><strong>🍗 ${nutrition.protein}g</strong><span>Protein</span></div><div class="nutrition-item nutrition-carbs"><strong>🌾 ${nutrition.carbs}g</strong><span>Carbohydrates</span></div><div class="nutrition-item nutrition-fat"><strong>🥑 ${nutrition.fat}g</strong><span>Fat</span></div><div class="nutrition-item nutrition-fibre"><strong>🌿 ${nutrition.fiber}g</strong><span>Fibre</span></div><div class="nutrition-item nutrition-sugar"><strong>🍯 ${nutrition.sugar}g</strong><span>Sugar</span></div></div>`;
}
